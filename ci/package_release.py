...
def parse_canonical_version(raw_version: str) -> Version:
    ...
    if str(version) != raw_version:
        raise ValueError(
            f"Use canonical PEP 440 version {version!s} instead of {raw_version!r}"
        )
    return version
...
def lower_bound(requirement: Requirement) -> Version | None:
    ...
    for specifier in requirement.specifier:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid lower bound in {requirement}: {specifier.version}"
            ) from exc
    return max(bounds) if bounds else None


def import_modules_from_pyproject(
    pyproject: dict, package_name: str
) -> tuple[str, ...]:
    hatch_packages = (
        pyproject.get("tool", {})
        .get("hatch", {})
    ...
    if isinstance(hatch_packages, list) and hatch_packages:
        return tuple(
            str(package).replace("/", ".").strip(".") for package in hatch_packages
        )
    ...
    includes = (
        setuptools_find.get("include") if isinstance(setuptools_find, dict) else None
    )
...
def load_workspace_packages(repo_root: Path = REPO_ROOT) -> dict[str, PackageInfo]:
    ...
    for member in members:
        ...
        missing_fields = [
            field for field in ("name", "version") if field not in project
        ]
        if missing_fields:
            joined_fields = ", ".join(missing_fields)
            raise ValueError(
                f"{pyproject_path} is missing required [project] field(s): {joined_fields}"
            )
...
def require_package(packages: dict[str, PackageInfo], package_name: str) -> PackageInfo:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown package {package_name!r}. Available packages: {available}"
        ) from exc
...
def dependent_packages(
    ...
    return sorted(
        (
            ...
            if canonicalize_name(package.name) != target_name
            and target_name in package.dependencies
        ),
...
def check_tag_available(
    repo_root: Path, package_name: str, version: Version
) -> list[str]:
...
def update_project_version(
    pyproject_path: Path, version: Version, *, dry_run: bool = False
) -> bool:
...
def update_dependency_lower_bound(
    ...
    *,
    ...
    if count != 1:
        raise ValueError(
            f"Unable to update dependency lower bound for {package_name} in {pyproject_path}"
        )
    if updated == content:
        return False
    if not dry_run:
...
def dependency_build_order(
    packages: dict[str, PackageInfo], target: PackageInfo
) -> list[PackageInfo]:
    ...
    def visit(package: PackageInfo) -> None:
        ...
        if normalized_name in visiting:
            raise ValueError(
                f"Cycle detected while resolving workspace dependencies for {package.name}"
            )
...
def single_artifact(
    output_dir: Path, pattern: str, label: str, package: PackageInfo
) -> Path:
...
def build_package_wheels(
    ...
    *,
    ...
    for package in packages_to_build:
        ...
        wheels[canonicalize_name(package.name)] = single_artifact(
            output_dir, "*.whl", "wheel", package
        )
    return wheels


def smoke_import_package(
    repo_root: Path, target: PackageInfo, wheels: dict[str, Path]
) -> None:
    with_args: list[str] = []
