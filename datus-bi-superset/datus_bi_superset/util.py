...
class DatasourceKey:
    ...
    def from_string(cls, datasource: str) -> DatasourceKey:
        ...
        if isinstance(datasource, dict):
            return cls(
                id=datasource.get("id", 0),
                type=DatasourceType(datasource.get("type", "table")),
            )
...
def normalize_time_column(
    form_data: dict[str, Any], query_object: QueryObject
) -> QueryObject:
    """
...
def flatten_operator(
    _form_data: dict[str, Any], _query_object: QueryObject
) -> PostProcessingRule:
...
def resample_operator(
    form_data: dict[str, Any], _query_object: QueryObject
) -> PostProcessingRule | None:
    ...
    return cast(
        PostProcessingRule,
        {
            ...
            "options": {
                ...
                "yearly_seasonality": form_data.get(
                    "forecastSeasonalityYearly", "auto"
                ),
                "weekly_seasonality": form_data.get(
                    "forecastSeasonalityWeekly", "auto"
                ),
...
def build_query_object(
    ...
    series_limit_metric = form_data.get("series_limit_metric") or form_data.get(
...
def build_default_query(
    base_query: QueryObject, _form_data: dict[str, Any]
) -> list[QueryObject]:
...
def build_timeseries_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_big_number_trendline_query(
    base_query: QueryObject, _form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_mixed_timeseries_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_pop_kpi_query(
    ...
    non_custom_shifts = [
        s
        for s in ensure_list(form_data.get("time_compare", []))
        if s not in ("custom", "inherit")
    ]
    custom_or_inherit_shifts = [
        s
        for s in ensure_list(form_data.get("time_compare", []))
        if s in ("custom", "inherit")
    ]
    ...
    base_query.post_processing = post_processing
    base_query.time_offsets = (
        time_offsets if is_time_comparison(form_data, base_query) else []
    )
...
def build_deck_scatter_query(
    ...
    metrics = []
    radius_value = (
        point_radius_fixed.get("value")
        if isinstance(point_radius_fixed, dict)
        else None
    )
    if radius_value:
...
def build_deck_screengrid_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_timeseries_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_time_pivot_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_country_map_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_world_map_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_parallel_coordinates_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_cal_heatmap_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
    ...
    if not base_query.extras:
        base_query.extras = {}
    base_query.extras["time_grain_sqla"] = time_grain_mapping.get(
        subdomain_granularity, "PT1M"
    )
...
def build_legacy_partition_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_paired_ttest_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_time_table_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_deck_multi_query(
    _base_query: QueryObject, _form_data: dict[str, Any]
) -> list[QueryObject]:
    """
...
def build_legacy_deck_geojson_query(
    base_query: QueryObject, form_data: dict[str, Any]
) -> list[QueryObject]:
    """
